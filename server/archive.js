...
class Archive {
  ...
  downloadArchiveFromS3(archiveId, individualOutput = false) {
    const params = {
      ...
      Key: `${this.opentok_project_id}/${archiveId}/archive.${
  ...
  listAvailableTranscripts() {
    return new Promise((resolve, reject) => {
      ...
      this.s3.listObjectsV2(params, (err, data) => {
        ...
        const archives = data.Contents.map(
          c => c.Key.split('/')[2].split('.')[0]
        )
  ...
  extractAudio(archiveId, vidStream) {
    return ffmpeg()
      ...
      .on('start', cmdline => {
        console.log(
          `Starting transcoding of archive ${archiveId}. Command: ${cmdline}`
        )
      })
      .on('error', err => {
              ...
              zipFile.openReadStream(entry, (err, readStream) => {
                ...
                wr.on('finish', () => {
                  ...
                  transcribe
                    ...
                    .then(txt => {
                      console.log(
                        `Transcription for ${entry.fileName}:\n${txt}\n`
                      )
                    ...
                    .then(() => {
                      ...
                      console.log(
                        `Uploaded transcript file to S3 for archive ${archiveId}`
                      )
                      zipFile.readEntry()
                    })
                    .catch(err => {
                      console.log(
                        `Error uploading transcript file to S3. Archive: ${archiveId}. Reason: ${err}`
                      )
                      zipFile.readEntry()
                    })
                })
